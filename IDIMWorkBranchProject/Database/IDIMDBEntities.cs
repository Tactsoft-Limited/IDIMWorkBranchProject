...
namespace IDIMWorkBranchProject.Data.Database
{
    public partial class IDIMDBEntities : DbContext
    {
        ...
        // Override SaveChanges to automatically apply auditing
        ...
        public override async Task<int> SaveChangesAsync()
        {
            ...
            if (UserExtention.IsAuthenticated())
            {
                userId = UserExtention.GetUserId(); // Adjust this based on how your system gets the authenticated user's ID
            }
            ...
            foreach (var entry in this.ChangeTracker.Entries<BaseEntity>())
            {
                ...
                {
                    // Set CreatedUser and CreatedDateTime for added entities
                    entry.Entity.CreatedUser = entry.Entity.CreatedUser ?? (int?)userId;
                    entry.Entity.CreatedDateTime = entry.Entity.CreatedDateTime ?? now;

                    // Initialize UpdateNo to 0 for new entities
                    entry.Entity.UpdateNo = entry.Entity.UpdateNo ?? 0; // Default to 0 if null
                }
                else if (entry.State == EntityState.Modified)
                {
                    // Set UpdatedUser and UpdatedDateTime for modified entities
                    entry.Entity.UpdatedUser = entry.Entity.UpdatedUser ?? (int?)userId;
                    entry.Entity.UpdatedDateTime = now;

                    // Increment the UpdateNo for modified entities
                    entry.Entity.UpdateNo = (entry.Entity.UpdateNo ?? 0) + 1;
                }
            }
        ...
        public override int SaveChanges()
        {
            ...
            // Retrieve the authenticated user ID if available
            if (UserExtention.IsAuthenticated())
            {
                userId = UserExtention.GetUserId(); // Adjust this based on how your system gets the authenticated user's ID
            }

            // Loop through all the entries in the ChangeTracker
            foreach (var entry in this.ChangeTracker.Entries<BaseEntity>())
            {
                if (entry.State == EntityState.Added)
                {
